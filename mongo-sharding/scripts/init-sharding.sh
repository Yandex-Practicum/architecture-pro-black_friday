...
wait_for_primary() {
  ...
  for _ in {1..60}; do
    result=$(docker compose exec -T "$service" mongosh --port "$port" --quiet --eval 'rs.isMaster().ismaster ? "true" : "false"' 2>/dev/null || true)
    if [[ "$result" == *"true"* ]]; then
      return 0
    fi
    sleep 1
  ...
  echo "Service $service did not become primary in time" >&2
  return 1
}
...
if (!initiated) {
  rs.initiate({
    _id: "rs-shard1",
    members: [{ _id: 0, host: "shard1-1:27019" }],
  })
}
EOF
...
wait_for_primary shard1-1 27019
...
if (!shards.includes("rs-shard1")) {
  db.adminCommand({ addShard: "rs-shard1/shard1-1:27019" })
}

if (!shards.includes("rs-shard2")) {
  db.adminCommand({ addShard: "rs-shard2/shard2-1:27019" })
}
