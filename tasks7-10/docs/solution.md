...
**Решение:** Перенести в Cassandra **заказы** (включая историю) и **корзины**, поскольку:
- Они генерируют подавляющую часть нагрузки в периоды распродаж.
...
#### 2.1. Заказы
Две таблицы для обслуживания основных запросов:
- **orders_by_user** – история заказов конкретного пользователя.
- **orders_by_id** – получение статуса заказа по его идентификатору.
...
CREATE TABLE orders_by_user (
    ...
    order_id UUID,
    items LIST<FROZEN<item>>,   -- FROZEN для неизменяемых вложенных UDT
    status TEXT,
...
Partition key: user_id – все заказы пользователя в одной партиции, запрос истории выполняется как локальное чтение одной партиции.

Clustering key: created_at DESC, затем order_id для уникальности. Позволяет эффективно извлекать последние заказы без дополнительной сортировки.

Равномерность: Каждый пользователь – отдельная партиция. В пиковые моменты партиции активных пользователей получают больше записей, но размер одной партиции ограничен (редко превышает несколько сотен заказов). Это не создаёт горячих точек, так как запросы распределены по пользователям.

Масштабируемость: Добавление узлов в кластер приводит к автоматическому перераспределению лишь небольшой части vnodes, без полной миграции данных.
...
```
Partition key: order_id – гарантирует быстрое точечное чтение статуса заказа.

Запись во время создания заказа идёт в обе таблицы через BATCH для атомарности.
...
1. Уровень консистентности записи: QUORUM – гарантирует, что данные записаны на большинство реплик, исключая потерю заказа при отказе одного узла.

2. Уровень консистентности чтения: QUORUM для статуса заказа, LOCAL_QUORUM для истории (допускает небольшую несвежесть). При использовании QUORUM автоматически включается Read Repair, поддерживающий актуальность часто запрашиваемых данных.

3. Anti-Entropy Repair: запуск nodetool repair с интервалом gc_grace_seconds (по умолчанию 10 дней) для гарантированного восстановления в случае долгих отказов. Для таблицы orders_by_id, где критична точность статуса, можно сократить интервал до 1 дня (с учётом частых обновлений статусов).

4. Hinted Handoff: включён по умолчанию; окно хранения подсказок можно оставить стандартным (3 часа).
...
### Итого
Для заказов, где потеря даже одного документа недопустима, комбинация QUORUM записи/чтения плюс относительно частый Anti-Entropy Repair минимизирует риск рассогласования. Для корзин, которые имеют TTL и часто перезаписываются, достаточно стандартного repair с подстраховкой Read Repair на каждое чтение – это баланс между надёжностью и производительностью.

Миграция заказов и корзин в Cassandra решает проблемы масштабирования, выявленные в MongoDB при использовании Range-Based Sharding. Consistent hashing и vnodes позволяют добавлять узлы без полного перераспределения данных, что критически важно в периоды пиковых нагрузок. Предложенная модель данных гарантирует быстрые чтения/записи, а многоуровневая стратегия восстановления целостности обеспечивает надёжность, сопоставимую с традиционными реляционными СУБД.