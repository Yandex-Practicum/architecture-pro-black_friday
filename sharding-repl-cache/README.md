...
В этой директории находится **финальное архитектурное решение** для первого блока заданий. Мы объединяем все изученные подходы:
1.  **Шардирование** (горизонтальное масштабирование).
2.  **Репликацию** (отказоустойчивость).
3.  **Кеширование** (высокая производительность чтения).
...
1.  Приложение сначала ищет данные в **Redis**.
2.  Если данных нет (Cache Miss), оно идет в **MongoDB (через mongos)**.
3.  Полученные данные сохраняются в Redis для будущих запросов.
...
```
⏱️ *Время выполнения: > 1.0 сек*

**Запрос 2 (Горячий кеш):** Данные берутся из Redis.
```bash
time curl -s "http://localhost:8080/helloDoc/users" > /dev/null
```
⏱️ *Время выполнения: < 0.05 сек (Мгновенно)*
