...
- zoned tag sharding
В mongodb можно создать логическую группу шардов и привязать к ней диапазон значений shard key
Этот метод позволит "изолировать" горячий шард от остальных выделив под него специальные шарды помощнее например, таким способом мы как бы резервируем ресурсы под горячий сегмент
