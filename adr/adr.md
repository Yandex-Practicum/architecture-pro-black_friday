...
|           Операция           | Должна читать primary | Может читать secondary |     Допустимая задержка репликации     | Обоснование                                                                                                                                                                       |
|:----------------------------:|:---------------------:|:----------------------:|:--------------------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       Создание заказа        |          да           |          нет           |                  ---                   | Чтение после создания нового заказа не может использовать устаревшие данные, так как это приведет к фрустрации и риску потери клиента - созданный заказ не отобразится немедленно |
| История заказов пользователя |          нет          |           да           |                 минуты                 | Как правило, история заказов пользователя востребована не сразу после создания нового заказа, поэтому даже длительная задержка репликации будет как правило незаметна             |
|  Отображение статуса заказа  |          нет          |           да           | менее минуты; если возможно -- секунды | Как правило, пользователь хочет убедиться, что заказ был оплачен; задержка репликации не должна быть дольше характерного времени проведения оплаты                                |
|                              |                       |                        |                                        |                                                                                                                                                                                   |
...
|    Операция     | Должна читать primary | Может читать secondary | Допустимая задержка репликации | Обоснование                                                                                                                                                                                                                                                                                                                    |
|:---------------:|:---------------------:|:----------------------:|:------------------------------:|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  Поиск товаров  |          нет          |           да           |   минуты (возможно, и менее)   | Поскольку остатки часто обновляются, а при этом хранятся в структуре товара, желательно подавать пользователю хотя бы относительно свежую информацию, слишком длительная задержка приведет к частому возникновению ситуаций, когда из-за задержки репликации пользователь не может купить товар, показанный как наличествующий |
| Описание товара |          нет          |           да           |   минуты (возможно, и более)   | Изменения в описаниях товаров - нечастое явление, а сама информация не является критически важной, допустимы большие задержки репликации                                                                                                                                                                                       |
|                 |                       |                        |                                |                                                                                                                                                                                                                                                                                                                                |
...
|     Операция     | Должна читать primary | Может читать secondary | Допустимая задержка репликации | Обоснование                                                                                                                                                                                          |
|:----------------:|:---------------------:|:----------------------:|:------------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Создание корзины |          да           |          нет           |              ---               | Чтение после создания корзины не может использовать реплику чтобы пользователь мог начать немедленно использовать корзину, противное влечет риск потери клиента                                      |
|  Чтение корзины  |          нет          |           да           |            секунды             | Чтение корзины часто применяется (автоматически) немедленно после помещения в нее нового товара, поэтому заметная задержка чтения может повлечь фрустрацию и потерю клиента -- "корзина не работает" |
| Слияние корзины  |          да           |          нет           |              ---               | Слияние гостевой корзины в пользовательскую должно производиться на primary-реплике чтобы исключить риск потери товара при слиянии                                                                   |
|                  |                       |                        |                                |                                                                                                                                                                                                      |
...
Sharding). При резком увеличении нагрузки (50 000 запросов/сек.) возникла высокая задержка при масштабировании: при
добавлении новых шардов MongoDB полностью перераспределяла данные между всеми узлами, что вызывало просадку latency
...
- Определить, какие части данных интернет-магазина (например, заказы, товары, корзины, история заказов, пользовательские
  сессии) являются критически важными с точки зрения целостности и скорости обработки.
- Обосновать, для каких именно данных применение Cassandra имеет смысл (учесть требования к масштабируемости,
  геораспределённости, скорости записи/чтения и целостности данных).

#### 10.2

- Разработать концептуальную модель для выбранных критически важных сущностей.
- Определить partition key и кластерные ключи для каждой сущности, объяснив, как они будут обеспечивать равномерное
  распределение данных и эффективный доступ в условиях экстремальной нагрузки.
- Обосновать выбор модели с точки зрения возможных «горячих» партиций и минимизации влияния решардинга при изменении
  топологии кластера.

#### 10.3

- Определить, какие стратегии (Hinted Handoff, Read Repair, Anti-Entropy Repair) можно применить для обеспечения
  целостности данных.
- Для каждой сущности выбрать стратегию.
- Обосновать выбор с учётом компромиссов между latency запросов и уровнем гарантии согласованности.

### Решение

#### 10.1

Критически важными с точки зрения целостности и скорости обработки являются следующие данные:

- заказы (текущие, т.е. недоставленные, но не история заказов);
- товары (текущие цены и остатки на складах, но не описания);
- корзины.

Целостность данных -- в целом слабая сторона Cassandra, тем не менее использование Cassandra оправдано для:

- заказов
    - так как большой наплыв заказов послужил причиной проблем на MongoDB, в то время как Cassandra позволит
      быстро и без остановки работы масштабировать БД;
    - так как создание нового заказа влечет за собой обновление остатков товаров, что желательно производить в пределах
      одной БД;
- товаров
    - так как большой наплыв просмотров послужил причиной проблем на MongoDB, в то время как Cassandra позволит
      быстро и без остановки работы масштабировать БД;
    - операция обновления остатков при покупке должна выполняться часто, быстро и с высокой степенью согласованности;
- корзин
    - так как большой наплыв операций с корзинами послужил причиной проблем на MongoDB, в то время как Cassandra
      позволит быстро и без остановки работы масштабировать БД;
...
status TEXT,                -- Статус заказа: 'new', 'processing', 'shipped', 'delivered' и т.д.
created_at TIMESTAMP,       -- Дата и время создания заказа
total_amount DECIMAL,       -- Общая сумма заказа
geo_zone TEXT,              -- Геозона заказа (например, "Москва", "Екатеринбург")
items LIST<FROZEN<MAP<TEXT, TEXT>>>, -- Список товаров: { "product_id": "...", "name": "...", "price": "...", "quantity": "..." }
...
category TEXT,                      -- Категория товара (например, "Электроника", "Книги")
name TEXT,                          -- Наименование товара
price DECIMAL,                      -- Цена товара
description TEXT,                   -- Описание товара (опционально, может храниться отдельно)
attributes MAP<TEXT, TEXT>,         -- Дополнительные атрибуты: цвет, размер и т.д.
stock_by_zone MAP<TEXT, INT>,       -- Остатки по геозонам: например, {"Москва": 50, "Екатеринбург": 30}
updated_at TIMESTAMP,               -- Время последнего обновления (для синхронизации)
...
CREATE TABLE IF NOT EXISTS carts (
 cart_id UUID,                       -- Уникальный идентификатор корзины
 user_id UUID,                       -- идентификатор клиента
 session_id UUID,                    -- идентификатор сессии
 status TEXT,                        -- Статус: "active", "ordered", "abandoned"
 items LIST<FROZEN<MAP<TEXT, TEXT>>>, -- Список товаров: { "product_id": "...", "quantity": "..." }
 created_at TIMESTAMP,
 updated_at TIMESTAMP,
 expires_at TIMESTAMP,               -- Для автоматической очистки
...
| Сущность |     Partition key     |         Cluster key          | Обоснование                                                                                                                                                                                                     |
|:--------:|:---------------------:|:----------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  Заказ   | Идентификатор клиента |        Статус заказа         | При удачном выборе идентификатора можно ожидать равномерного распределения по шардам, статус заказа как кластерный ключ позволит быстро находить наиболее востребованные виды заказов -- новые и недоставленные |
|  Товар   | Идентификатор товара  |          Категория           | При удачном выборе идентификатора можно ожидать равномерного распределения по шардам, категория товара как кластерный ключ позволит быстро искать товары по категориям (наиболее востребованный поиск)          |
| Корзина  | Идентификатор корзины | Идентификатор клиента/сессии | При удачном выборе идентификатора можно ожидать равномерного распределения по шардам, идентификатор клиента/сессии как кластерный ключ позволит быстро искать корзины по принадлежности                         |                                                                                                                                                                                                                 |

При правильном выборе идентификатора (можно ожидать использование UUID) "горячие" партиции не должны возникать.

#### 10.3

| Сущность | Стратегия целостности | Обоснование                                                                                                    |
|:--------:|:---------------------:|:---------------------------------------------------------------------------------------------------------------|
|  Заказ   |    Hinted Handoff     | Можно ожидать, что недоступные серверы будут быстро восстанавливаться, а заказы читаются сравнительно нечасто  |
|  Товар   |      Read repair      | Критически важна согласованность данных по остаткам (частые чтения, частые записи)                             |
| Корзина  |    Hinted Handoff     | Можно ожидать, что недоступные серверы будут быстро восстанавливаться, а корзины читаются сравнительно нечасто |

### Принимая во внимание ограничения

### Принимая во внимание ограничения

1. **Ограниченная поддержка сложных запросов**  
   В отличие от MongoDB, Cassandra не поддерживает гибкие запросы с фильтрацией по произвольным полям, агрегации или
   JOIN’ы. Это означает, что все запросы должны быть заранее спроектированы под конкретные случаи.
2. **Сложность обеспечения строгой согласованности**  
   Даже при использовании `Read Repair` и `Hinted Handoff` возможны временные расхождения данных.
3. **Риск «горячих» партиций при неравномерном распределении активности**  
   Несмотря на использование UUID для ключей, в реальных сценариях может возникнуть ситуация, когда отдельные клиенты (
   например, оптовые покупатели) генерируют аномально высокую нагрузку.
4. **Сложность миграции данных и синхронизации с существующей MongoDB**  
   Переход на Cassandra требует полной переработки модели данных, логики доступа и механизмов транзакций.
5. **Высокие требования к квалификации персонала**  
   Настройка и мониторинг кластера Cassandra (включая стратегии репликации, управление gossip, repair) требует глубоких
   знаний.