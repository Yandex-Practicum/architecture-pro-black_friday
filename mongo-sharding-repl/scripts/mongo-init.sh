...
echo "--- Проверка shard1 (shard1-1:27018) ---"
COUNT_SHARD1=$(docker compose exec -T shard1-1 mongosh --port 27018 --quiet <<EOF
use somedb
...
echo "--- Проверка shard2 (shard2-1:27019) ---"
COUNT_SHARD2=$(docker compose exec -T shard2-1 mongosh --port 27019 --quiet <<EOF
use somedb
db.helloDoc.countDocuments()
EOF
)
