...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", {}):
            ...
            try:
                shard_conn_str = shard_host.split("/")[1] if "/" in shard_host else shard_host
                shard_client = motor.motor_asyncio.AsyncIOMotorClient(f"mongodb://{shard_conn_str}")
                ...
                logger.warning(f"Could not get replica status for shard {shard_id}: {e}")
                shard_replicas[shard_id] = {"error": str(e)}
        
        try:
            collection_stats = await db.command("collStats", "helloDoc")
            if "shards" in collection_stats:
                shard_distribution = {}
                for shard_id, shard_stats in collection_stats["shards"].items():
                    shard_distribution[shard_id] = {
                        "count": shard_stats.get("count", 0),
                        "size": shard_stats.get("size", 0)
                    }
        except Exception as e:
            logger.warning(f"Could not get shard distribution: {e}")
            shard_distribution = None
